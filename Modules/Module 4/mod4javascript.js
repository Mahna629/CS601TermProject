...
function validate() {
  ...
  var facilitators = ["Fazil", "Christian", "Josh"];
  var validPage = true;
  ...
  // check for different type of validation on the third form
  if (!emptyCheck(facilitator)) {
    validPage = false;
  } else if (facilitators.indexOf(facilitator.value) == -1) {
    var message = "That is not a valid facilitator!";
    DOMError(facilitator, message);
    validPage = false;
  } else {
    removeDOMError(facilitator);
  }
...
function lengthCheck(node) {
  if (node.value.length > 0 && node.value.length <= 2) {
    var message = "Name needs to be longer than 2 characters!";
    DOMError(node, message);
    return false;
  } else {
    return true;
